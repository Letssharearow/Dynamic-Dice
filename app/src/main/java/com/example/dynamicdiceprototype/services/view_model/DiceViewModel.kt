...
class DiceViewModel(
    // dataStore
    private val imagesStore: DataStore<ImageDTOMap>,
    private val dicesStore: DataStore<DiceDTOMap>,
    private val userConfigStore: DataStore<UserDTO>,
    // needed for bitmap
  ...
  private fun initiateImages() {
    val images = getInitialImages(resources)
    saveImages(images)
  }
    ...
    diceInEdit =
        Dice.random(
            imageMap.toMutableMap().also {
              it[IMAGE_DTO_NUMBER_CONTENT_DESCRIPTION] =
                  ImageDTO(
  ...
  private fun removeKeyFromInnerMaps(
      diceGroups: MutableMap<String, DiceGroup>,
      keyToRemove: String
  ) {
  ...
  private fun saveGroup(group: DiceGroup) {
    viewModelScope.launch {
  ...
  private fun setTempGroup(group: DiceGroup) {
  ...
  fun selectDiceGroup(group: DiceGroup) {
    ...
    try {
      ...
      Log.d(LOG_TAG, "Dice is null ${group.dices.keys}")
    }
  }
  ...
  private fun saveToHistory(rollId: Int, sum: Int, dices: List<Dice>) {
    history.add(
        0,
        RollState(
            rollId = rollId,
  ...
  fun resetRollingScreen() {
    ...
    if (history.isNotEmpty()) {
      history.add(0, RollState(unlockedDicesCount = 0, isEmpty = true))
    }
    countRolls = 0
    currentSum = 0
