...
fun LandingPage(
  ...
  Log.d(LOG_TAG, "Recompose LandingPage ${dices.size}")
      ...
      modifier = modifier.fillMaxSize()) {
        ...
        Column(modifier = if (showHistory) modifier.weight(1f) else Modifier) {
          Footer(
              viewModel,
              onRollClicked,
              onActionClicked = { showAddDiceDialog = true },
              onHistoryClicked = { showHistory = !showHistory },
          )
          if (showHistory) {
    ...
    Column(
        ...
        modifier = Modifier.align(Alignment.CenterStart)) {
          ...
          PopMenuWithAlert(
              ...
              items =
                  listOf(
                      MenuItem(text = "Add / Remove dice", callBack = { onActionClicked() }),
                      MenuItem(
                          text = "Lock all dice",
